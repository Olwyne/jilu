    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14, marginBottom: 30 }}>
        {[
          ...
          [String(filmsSeen), t('stats.filmsSeen'), 'var(--color-pink)'],
      ...
      <div style={{ padding: 22, borderRadius: 18, background: 'var(--color-surface)', border: '1px solid var(--color-border)', marginBottom: 30 }}>
        <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, margin: '0 0 4px' }}>{t('stats.recentActivity')}</h3>
        <div style={{ fontSize: 12.5, color: 'var(--color-muted-2)', marginBottom: 18 }}>{t('stats.recentActivitySub')}</div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120 }}>
          {weeks.slice().reverse().map((v, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%', gap: 6 }}>
              ...
              <span style={{ fontSize: 10, color: 'var(--color-muted-3)' }}>{i === 9 ? t('stats.thisWeek') : (i % 3 === 0 ? `S-${9 - i}` : '')}</span>
            </div>
          ))}
        </div>
      </div>
