...
export default function EpisodeModal({ work, sNum, eNum, ratings, feed, actions, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.68)', zIndex: 70, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '7vh 20px 20px', overflowY: 'auto' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 540, background: 'var(--color-modal-bg)', border: '1px solid var(--color-border-btn)', borderRadius: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 15, padding: '20px 22px', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <div onClick={onClose} style={{ width: 34, height: 34, borderRadius: 10, border: '1px solid var(--color-border-btn)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 18 }}>✕</div>
        </div>
        <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[1, 2, 3, 4, 5].map((n) => (
              <span key={n} onClick={() => actions.setRating('e', key, n)} style={{ fontSize: 30, cursor: 'pointer', color: n <= myRating ? 'var(--color-gold)' : 'var(--color-check-border)' }}>{n <= myRating ? '★' : '☆'}</span>
            ))}
          </div>
        </div>
