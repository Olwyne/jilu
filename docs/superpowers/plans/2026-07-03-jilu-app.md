...
- App name is **Jilu** everywhere (page title, header brand mark, README) — the original prototype's "Kōra" branding is replaced 1:1, no other product-name references.
- Visual source of truth for every screen is `extracted/application-web-de-suivi-culturel/project/Kora.dc.html` — copy exact colors, spacing, font sizes, radii from its inline `style="..."` strings. Do not invent new values.
...
Values copied from the `const C` object and repeated literals in `Kora.dc.html` (lines 636-639 and throughout):
...
Port the `<style>` block from `Kora.dc.html` lines 15-23 verbatim (box-sizing reset, html/body background, scrollbar styling, `koraPulse`/`koraIn`/`koraToast` keyframes — rename them `jiluPulse`/`jiluIn`/`jiluToast` since the "kora" naming is prototype-specific):
...
Ported directly from `Kora.dc.html` lines 636-680, unchanged logic (only the module wrapper and `export` keywords are new):
...
Palette ported verbatim from `Kora.dc.html` line 640-644 (`POSTERS` array):
...
Structure/copy/icons ported from `Kora.dc.html` lines 28-54 (the `<aside>` block) — same six SVG icons (dashboard/library/calendar/stats/feed + search entry), same "Cette semaine" stat card at the bottom (values passed in as `weekWatched`/`toCatch` props, computed by the parent from `data`).
...
Same 5-item bottom bar as `Kora.dc.html` lines 528-536 (icons + short labels "Accueil/Biblio/Calendrier/Journal/Stats"), fixed to viewport bottom, only rendered when `isMobile` is true. Test asserts clicking an icon calls `setView`.
...
- Produces: category/status/sort filter chips (local `useState`, not persisted — matches original's transient `category`/`status`/`sort` state), a responsive grid of `WorkCard`s. Ported from `Kora.dc.html` lines 79-124 and the `renderVals()` library-branch, lines 1097-1139.
...
`LibraryView.module.css` — chip styles ported from the original `chip()`/`smallChip()` string builders (`Kora.dc.html` lines 1022-1027):
...
- Produces: full detail screen ported from `Kora.dc.html` lines 126-258 (poster/overview/rating/status-cycle header, optional `GamePanel` for `jeux`, `JournalThread` for work-level comments, `SeasonList` for episode checklists) minus the community-comments/community-rating parts (cut per Assumption 5).
...
Ported from `Kora.dc.html` lines 160-190, minus the pre-filled duration cards (Assumption 2) — keeps the hour counter and the three completion checkboxes.
...
- Produces: modal ported from `Kora.dc.html` lines 554-607, minus the community-rating/community-comments block (Assumption 5) — shows only "Ma note" (own star rating for the episode) and reuses `JournalThread` for the comment list.
...
- Produces: "À rattraper" (aired-but-unwatched, grouped one row per work) and "À venir" (next unaired episode per work, sorted soonest-first) sections, ported from `Kora.dc.html` lines 262-301 and the calendar branch of `renderVals()` (lines 1217-1271).
...
- Produces: debounced (400ms) live search across all 5 sources, min 2 chars, "+ Ajouter" / "Ajouté ✓" button per result — ported from `Kora.dc.html` lines 609-630, with the hardcoded `catalog` array (lines 1503-1514) replaced by the real aggregator.
...
- Produces: bottom toast ported from `Kora.dc.html` lines 539-552, auto-dismiss after 6s (same timing as the original's `setTimeout(...,6000)` in line 978), with a "Noter" button that opens the `EpisodeModal` for that episode.
